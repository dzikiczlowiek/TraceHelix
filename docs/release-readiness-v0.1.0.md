...
| P1-11 | No self-contained, tested production distribution | open-deferred | There is no published self-contained binary, installer, service unit, signed checksum, or release workflow; the API does not serve `web/dist` (Vite dev proxy only); the training exporter is invoked through `uv`/PATH rather than bundled. The compose images are local build targets, not published artifacts. | `v0.1.0` is a source release to build from, not an installable product. Published-artifact end-to-end testing is a follow-up. |
