...
Deferred capabilities include browser upload, arbitrary source-file browsing, sequence zoom/alignment, learned classifiers, ONNX inference, and live AI integration. The source-built production topology has bounded Chromium acceptance coverage through `scripts/verify-browser.sh`, together with a deterministic local source-bundle and local install-from-artifact evidence. A public release, downloaded-public-artifact verification, and signing evidence remain pending.
